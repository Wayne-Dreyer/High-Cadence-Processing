...
#NOTE below import statements are commented out when this is tested in isolation as the files mentioned will not be accessible.
#The calls to their functions in the 'runclicked' event handler must also be commented out when testing.
...
class InputGUI(tk.Frame):
    def __init__(self, parent):
        ...
        def runclicked():
            ...
            #NOTE DEBUG CODE
            tk.messagebox.showinfo("Flag", "{}".format(selectedflag))
            #NOTE END OF DEBUG CODE
            #NOTE DEBUG CODE
            tk.messagebox.showinfo("FrameSizes", "inputSelect\nWidth:{}\nHeight:{}\n\nflag\nWidth:{}\nHeight:{}\n\nrun\nWidth:{}\nHeight:{}".format(inputSelectFrame.winfo_width(), inputSelectFrame.winfo_height(), flagFrame.winfo_width(), flagFrame.winfo_height(), runFrame.winfo_width(), runFrame.winfo_height()))
            #NOTE END OF DEBUG CODE
            if(selectedflag == ""):
                ...
                try:
                    ...
                    if(len(fitfilename) == 0):
                        ...
                        #NOTE DEBUG CODE
                        tk.messagebox.showinfo("Values", "Fit:{}".format(fitfilename))
                        #NOTE END OF DEBUG CODE
...
#                                try:
                            if(selectedflag == 'Photoplot'): #This flag requires no additional info
                                tk.messagebox.showinfo("Success", "Success, Processing will now begin")
#                                   processData(data) #passes the data to the next component
                                ...
                                tk.messagebox.showinfo("Success", "Success, Processing will now begin")
#                                   processData(data) #passes the data to the next component
                            elif(selectedflag == 'CheckPixels'): #TODO add debug msg below #We need to get coordinates and zoom factor to zoom
                                #NOTE DEBUG CODE
                                tk.messagebox.showinfo("Success", "Success, Check Pixels Prompt will now open")
                                #NOTE END OF DEBUG CODE
...
#                                   processData(data) #passes the data to the next component
                                ...
                                #NOTE DEBUG CODE
                                tk.messagebox.showinfo("Success", "Success, Zoomed Prompt will now open")
                                #NOTE END OF DEBUG CODE
#                                   guiHandler = GUIHandler.getInstance()
#                                   guiHandler.setZoomedPromptData(data)
#                                   guiHandler.setWindow("ZoomedPrompt")
                            elif(selectedflag == 'Orbits'):
                                #NOTE DEBUG CODE
                                tk.messagebox.showinfo("Success", "Success, Orbit Prompt will now open")
                                #NOTE END OF DEBUG CODE
#                                   guiHandler = GUIHandler.getInstance()
...
#                               tk.messagebox.showerror("Invalid Type", "An input was an invalid type".format(itemsg))
...
#                               tk.messagebox.showerror("Error", "An error occurred while opening the files.\n{}".format(e2))
        ...
        def limitresourcesclicked():
            #NOTE DEBUG CODE
            tk.messagebox.showinfo("Success", "Success, Resources Window will now open")
            #NOTE END OF DEBUG CODE
            #guiHandler = GUIHandler.getInstance()
